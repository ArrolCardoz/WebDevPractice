...
app.post("/", async (req, res) => {
  ...
  if (req.body.type == "" && req.body.participants == "") {
    ...
    try {
      ...
      var ran_key = Math.floor(Math.random() * result.length);
      console.log(result[ran_key]);

      res.render("index.ejs", { data: result[ran_key] });
    } catch (error) {
      console.error("Failed to make request:", error.message);
      if (error.message == "Request failed with status code 404")
        res.render("index.ejs", {
          error: "Don't have any activity with this combimantion",
        });
      else
        res.render("index.ejs", {
          error: error.message,
